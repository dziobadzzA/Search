...
def resolve(args):
    ...
    if args.stream:
        ...
        if args.save:
            bckup = sqlite3.connect('file:ip2nation.db',detect_types=sqlite3.PARSE_DECLTYPES,uri=True)
            with bckup:
                ipr.conn.backup(bckup)
        else:
...
resolve_parser.add_argument('save',  help="When set to true saves the db", action="store_true")  # add the name argument
